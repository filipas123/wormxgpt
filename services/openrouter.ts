...
class OpenRouterService {
  ...
  constructor() {
    const saved = typeof window !== 'undefined' ? localStorage.getItem('openRouterApiKey') : null;
    if (saved) {
      this.apiKey = saved;
    }
  }
  ...
  getApiKey(): string {
    return this.apiKey || (typeof window !== 'undefined' ? localStorage.getItem('openRouterApiKey') : '') || '';
  }
    ...
    try {
      const response = await fetch(this.baseUrl + '/chat/completions', {
        ...
        headers: {
          ...
          'HTTP-Referer': 'https://wormgpt.local',
  ...
  async *streamChat(settings: AppSettings, messages: Message[], signal?: AbortSignal): AsyncGenerator<{ text: string; images: string[]; video?: string; audio?: string; sources?: { title: string; url: string }[]; toolInvocations?: ToolInvocation[] }> {
    ...
    const requestBody: any = {
      model: settings.model || 'meta-llama/llama-3-70b-instruct',
    ...
    // instead of leaving only the inline status line as evidence.
    ...
    try {
      for (let turn = 0; turn < MAX_TURNS; turn++) {
        ...
        const response = await fetch(url, {
          ...
          body: JSON.stringify({
            ...requestBody,
            messages: prunedMessages, // Use prunedMessages for the current request
            top_p: settings.topP ?? 1.0,
            ...(settings.maxTokens ? { max_tokens: settings.maxTokens } : {})
          })
        });
        ...
        if (!response.ok) {
          ...
          if (requestBody?.tools && !toolFallbackUsed && /tool|function/i.test(errorText)) {
            ...
            console.warn('[OpenRouter] Tool declarations rejected — retrying without tools.');
            continue;
          }
