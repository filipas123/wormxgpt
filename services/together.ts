...
class TogetherService {
  ...
  constructor() {
    const saved = typeof window !== 'undefined' ? localStorage.getItem('togetherApiKey') : null;
    if (saved) {
      this.apiKey = saved;
    }
  }
  ...
  getApiKey(): string {
    return this.apiKey || (typeof window !== 'undefined' ? localStorage.getItem('togetherApiKey') : '') || '';
  }
  ...
  async *streamChat(settings: AppSettings, messages: Message[], signal?: AbortSignal): AsyncGenerator<{ text: string; images: string[]; video?: string; audio?: string; sources?: { title: string; url: string }[]; toolInvocations?: ToolInvocation[] }> {
    ...
    const requestBody: any = {
      model: settings.model || 'meta-llama/Llama-3.3-70B-Instruct-Turbo',
      messages: apiMessages,
      temperature: settings.temperature,
      stream: true,
      ...(settings.maxTokens ? { max_tokens: settings.maxTokens } : {}),
      top_p: settings.topP ?? 1.0,
      repetition_penalty: (settings.presencePenalty ?? 0.0) + 1.0, // Together uses repetition_penalty
    };
    ...
    // This `currentApiMessages` will be updated and sent in each turn
    ...
    try {
      for (let turn = 0; turn < MAX_TURNS; turn++) {
        ...
        // Context Pruning: Keep system message + last N messages to prevent "Too Long" errors
        const attachedCount = settings.attachedMessagesCount || 8;
        let recentMsgs = currentApiMessages.slice(-attachedCount);
        
        // Ensure we don't start with a 'tool' role or a 'model' role that is just tool_calls
        ...
        const prunedApiMessages = systemMsg ? [systemMsg, ...recentMsgs.filter(m => m !== systemMsg)] : recentMsgs;
        ...
        if (!response.ok) {
          ...
          if (requestBody?.tools && !toolFallbackUsed && /tool|function/i.test(errorText)) {
            ...
            console.warn('[Together AI] Tool declarations rejected — retrying without tools.');
            continue;
          }
        ...
        let turnToolCalls: Record<number, { id: string; name: string; args: string; thought_signature?: string }> = {};
                ...
                if (parsed.choices?.[0]?.delta?.tool_calls) {
                  ...
                  for (const tc of parsed.choices[0].delta.tool_calls) {
                    ...
                    if (!turnToolCalls[idx]) {
                      turnToolCalls[idx] = { id: '', name: '', args: '', thought_signature: undefined };
                    }
        ...
        if (isMakingToolCall && Object.keys(turnToolCalls).length > 0) {
          ...
          currentApiMessages.push({
            ...
            tool_calls: toolCallsArray.map(tc => {
              const argsObj = validateAndFixToolArgs(tc.args, tc.name);
              return {
                id: tc.id,
                type: 'function',
                function: { name: tc.name, arguments: argsObj },
                thought_signature: tc.thought_signature || `Executing ${tc.name} with provided arguments`
              };
            })
          } as any);
      ...
      console.error('Together AI stream error:', error);
      throw new Error(error.message || 'Failed to communicate with Together AI API');
    }
  }
}
